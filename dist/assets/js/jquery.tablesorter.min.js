...
function(g) {
		...
		tablesorter: new function() {
			...
			function p(b, a, c) {
				...
				var h, e = b.config,
					s = e.textExtraction || "",
					k = "",
					k = "basic" === s ? g(a).attr(e.textAttribute) || a.textContent || a.innerText || g(a).text() || "" : "function" === typeof s ? s(a, b, c) : "function" === typeof(h = f.getColumnData(b, s, c)) ? h(a, b, c) : a.textContent || a.innerText || g(a).text() || "";
				return g.trim(k)
			}
			function u(b) {
				var a = b.config,
					c = a.$tbodies = a.$table.children("tbody:not(." + a.cssInfoBlock + ")"),
					h, e, s, k, l, g, r, n, x = 0,
					q = "",
					t = c.length;
				if (0 === t) return a.debug ? d("Warning: *Empty table!* Not building a parser cache") : "";
				a.debug && (n = new Date, d("Detecting parsers for each column"));
				for (e = []; x < t;) {
					h = c[x].rows;
					if (h[x]) for (s = a.columns, k = 0; k < s; k++) {
						l = a.$headers.filter('[data-column="' + k + '"]:last');
						g = f.getColumnData(b, a.headers, k);
						r = f.getParserById(f.getData(l, g, "sorter"));
						a.empties[k] = f.getData(l, g, "empty") || a.emptyTo || (a.emptyToBottom ? "bottom" : "top");
						a.strings[k] = f.getData(l, g, "string") || a.stringTo || "max";
						if (!r) a: {
							l = b;
							g = h;
							r = -1;
							for (var u = k, A = void 0, y = f.parsers.length, z = !1, F = "", A = !0;
							"" === F && A;) r++, g[r] ? (z = g[r].cells[u], F = p(l, z, u), l.config.debug && d("Checking if value was empty on row " + r + ", column: " + u + ': "' + F + '"')) : A = !1;
							for (; 0 <= --y;) if ((A = f.parsers[y]) && "text" !== A.id && A.is && A.is(F, l, z)) {
								r = A;
								break a
							}
							r = f.getParserById("text")
						}
						a.debug && (q += "column:" + k + "; parser:" + r.id + "; string:" + a.strings[k] + "; empty: " + a.empties[k] + "\n");
						e.push(r)
					}
					x += e.length ? t : 1
				}
				a.debug && (d(q ? q : "No parsers detected"), v("Completed detecting parsers", n));
				a.parsers = e
			}
			function y(b) {
				var a, c, h, e, s, k, l, m, r, n, x, q = b.config,
					t = q.$table.children("tbody"),
					u = q.parsers;
				q.cache = {};
				if (!u) return q.debug ? d("Warning: *Empty table!* Not building a cache") : "";
				q.debug && (m = new Date);
				q.showProcessing && f.isProcessing(b, !0);
				for (s = 0; s < t.length; s++) if (x = [], a = q.cache[s] = {
					normalized: []
				}, !t.eq(s).hasClass(q.cssInfoBlock)) {
					r = t[s] && t[s].rows.length || 0;
					for (h = 0; h < r; ++h) if (n = {
						child: []
					}, k = g(t[s].rows[h]), l = [], k.hasClass(q.cssChildRow) && 0 !== h) c = a.normalized.length - 1, a.normalized[c][q.columns].$row = a.normalized[c][q.columns].$row.add(k), k.prev().hasClass(q.cssChildRow) || k.prev().addClass(f.css.cssHasChild), n.child[c] = g.trim(k[0].textContent || k[0].innerText || k.text() || "");
					else {
						n.$row = k;
						n.order = h;
						for (e = 0; e < q.columns; ++e)"undefined" === typeof u[e] ? q.debug && d("No parser found for cell:", k[0].cells[e], "does it have a header?") : (c = p(b, k[0].cells[e], e), c = u[e].format(c, b, k[0].cells[e], e), l.push(c), "numeric" === (u[e].type || "").toLowerCase() && (x[e] = Math.max(Math.abs(c) || 0, x[e] || 0)));
						l[q.columns] = n;
						a.normalized.push(l)
					}
					a.colMax = x
				...
				q.debug && v("Building cache for " + r + " rows", m)
			}
			function z(b, a) {
				var c = b.config,
					h = c.widgetOptions,
					...
					k = c.cache,
					d, m, r, p, x, q;
				if (n(k)) return c.appender ? c.appender(b, s) : b.isUpdating ? c.$table.trigger("updateComplete", b) : "";
				c.debug && (q = new Date);
				for (x = 0; x < e.length; x++) if (d = g(e[x]), d.length && !d.hasClass(c.cssInfoBlock)) {
					r = f.processTbody(b, d, !0);
					d = k[x].normalized;
					m = d.length;
					for (p = 0; p < m; p++) s.push(d[p][c.columns].$row), c.appender && (!c.pager || c.pager.removeRows && h.pager_removeRows || c.pager.ajax) || r.append(d[p][c.columns].$row);
					f.processTbody(b, r, !1)
				}
				c.appender && c.appender(b, s);
				c.debug && v("Rebuilt table", q);
			...
			function D(b) {
				...
				m.$headers.each(function(d) {
					c = g(this);
					a = f.getColumnData(b, m.headers, d, !0);
					m.headerContent[d] = g(this).html();
					s = m.headerTemplate.replace(/\{content\}/g, g(this).html()).replace(/\{icon\}/g, e);
					m.onRenderTemplate && (h = m.onRenderTemplate.apply(c, [d, s])) && "string" === typeof h && (s = h);
					g(this).html('<div class="' + f.css.headerIn + '">' + s + "</div>");
				...
				B(b);
				m.debug && (v("Built headers:", l), d(m.$headers))
			}
			function E(b, a, c) {
				var h = b.config;
			...
			function B(b) {
				var a, c, h = b.config;
				h.$headers.each(function(e, s) {
					c = g(s);
					a = "false" === f.getData(s, f.getColumnData(b, h.headers, e, !0), "sorter");
					s.sortDisabled = a;
			...
			function G(b) {
				var a, c, h = b.config,
					e = h.sortList,
					s = e.length,
					d = f.css.sortNone + " " + h.cssNone,
					l = [f.css.sortAsc + " " + h.cssAsc, f.css.sortDesc + " " + h.cssDesc],
					m = ["ascending", "descending"],
					r = g(b).find("tfoot tr").children().add(h.$extraHeaders).removeClass(l.join(" "));
				h.$headers.removeClass(l.join(" ")).addClass(d).attr("aria-sort", "none");
				for (a = 0; a < s; a++) if (2 !== e[a][1] && (b = h.$headers.not(".sorter-false").filter('[data-column="' + e[a][0] + '"]' + (1 === s ? ":last" : "")), b.length)) {
					for (c = 0; c < b.length; c++) b[c].sortDisabled || b.eq(c).removeClass(d).addClass(l[e[a][1]]).attr("aria-sort", m[e[a][1]]);
					r.length && r.filter('[data-column="' + e[a][0] + '"]').removeClass(d).addClass(l[e[a][1]])
				}
				h.$headers.not(".sorter-false").each(function() {
					var b = g(this),
						a = this.order[(this.count + 1) % (h.sortReset ? 3 : 2)],
						a = b.text() + ": " + f.language[b.hasClass(f.css.sortAsc) ? "sortAsc" : b.hasClass(f.css.sortDesc) ? "sortDesc" : "sortNone"] + f.language[0 === a ? "nextAsc" : 1 === a ? "nextDesc" : "nextNone"];
					b.attr("aria-label", a)
				})
			}
			function L(b) {
			...
			function M(b, a) {
				var c, h, e, f, d, l = b.config,
					m = a || l.sortList;
				l.sortList = [];
				g.each(m, function(b, a) {
					f = parseInt(a[0], 10);
					if (e = l.$headers.filter('[data-column="' + f + '"]:last')[0]) {
						h = (h = ("" + a[1]).match(/^(1|d|s|o|n)/)) ? h[0] : "";
						switch (h) {
						case "1":
						case "d":
							h = 1;
						...
						case "s":
							...
							break;
						case "o":
							c = e.order[(d || 0) % (l.sortReset ? 3 : 2)];
							h = 0 === c ? 1 : 1 === c ? 0 : 2;
							break;
						case "n":
							e.count += 1;
							h = e.order[e.count % (l.sortReset ? 3 : 2)];
							break;
						default:
							h = 0
						}
						...
						c = [f, parseInt(h, 10) || 0];
						l.sortList.push(c);
						h = g.inArray(c[1], e.order);
						e.count = 0 <= h ? h : c[1] % (l.sortReset ? 3 : 2)
					}
				})
			}
			function N(b, a) {
			...
			function O(b, a, c) {
				var h, e, d, k = b.config,
					l = !c[k.sortMultiSortKey],
					m = k.$table;
				...
				if (l) {
					k.sortList = [];
					if (null !== k.sortForce) for (h = k.sortForce, c = 0; c < h.length; c++) h[c][0] !== e && k.sortList.push(h[c]);
					h = a.order[a.count];
					if (2 > h && (k.sortList.push([e, h]), 1 < a.colSpan)) for (c = 1; c < a.colSpan; c++) k.sortList.push([e + c, h])
				} else {
					if (k.sortAppend && 1 < k.sortList.length) for (c = 0; c < k.sortAppend.length; c++) d = f.isValueInArray(k.sortAppend[c][0], k.sortList), 0 <= d && k.sortList.splice(d, 1);
					if (0 <= f.isValueInArray(e, k.sortList)) for (c = 0; c < k.sortList.length; c++) d = k.sortList[c], h = k.$headers.filter('[data-column="' + d[0] + '"]:last')[0], d[0] === e && (d[1] = h.order[a.count], 2 === d[1] && (k.sortList.splice(c, 1), h.count = -1));
					else if (h = a.order[a.count], 2 > h && (k.sortList.push([e, h]), 1 < a.colSpan)) for (c = 1; c < a.colSpan; c++) k.sortList.push([e + c, h])
				}
				if (null !== k.sortAppend) for (h = k.sortAppend, c = 0; c < h.length; c++) h[c][0] !== e && k.sortList.push(h[c]);
			...
			function I(b) {
				...
				if (!t.serverSideSorting && !n(t.cache)) {
					t.debug && (d = new Date);
					for (c = 0; c < z; c++) k = t.cache[c].colMax, g = t.cache[c].normalized, g.sort(function(c, d) {
						for (a = 0; a < y; a++) {
							e = w[a][0];
							m = w[a][1];
							q = 0 === m;
							if (t.sortStable && c[e] === d[e] && 1 === y) break;
						...
						return c[t.columns].order - d[t.columns].order
					});
					t.debug && v("Sorting on " + w.toString() + " and dir " + m + " time", d)
				}
			}
			function J(b, a) {
			...
			function K(b) {
				...
				c.unbind("sortReset update updateRows updateCell updateAll addRows updateComplete sorton appendCache updateCache applyWidgetId applyWidgets refreshWidgets destroy mouseup mouseleave ".split(" ").join(a.namespace + " ")).bind("sortReset" + a.namespace, function(c, e) {
					...
					var r = g.fn.closest ? m.closest("tr") : m.parents("tr").filter(":first");
					e = m[0];
					l.length && 0 <= h && (l = l.eq(h).find("tr").index(r), m = m.index(), a.cache[h].normalized[l][a.columns].$row = r, e = a.cache[h].normalized[l][m] = a.parsers[m].format(p(b, e, m), b, e, m), "numeric" === (a.parsers[m].type || "").toLowerCase() && (a.cache[h].colMax[m] = Math.max(Math.abs(e) || 0, a.cache[h].colMax[m] || 0)), H(c, d, f))
				}).bind("addRows" + a.namespace, function(h, e, d, f) {
					h.stopPropagation();
					b.isUpdating = !0;
					if (n(a.cache)) B(b), E(b, d, f);
					else {
						e = g(e);
						var l, m, r, v, x = e.filter("tr").length,
							q = c.find("tbody").index(e.parents("tbody").filter(":first"));
						a.parsers && a.parsers.length || u(b);
						for (h = 0; h < x; h++) {
							m = e[h].cells.length;
							v = [];
							r = {
								child: [],
								$row: e.eq(h),
								order: a.cache[q].normalized.length
							};
							for (l = 0; l < m; l++) v[l] = a.parsers[l].format(p(b, e[h].cells[l], l), b, e[h].cells[l], l), "numeric" === (a.parsers[l].type || "").toLowerCase() && (a.cache[q].colMax[l] = Math.max(Math.abs(v[l]) || 0, a.cache[q].colMax[l] || 0));
							v.push(r);
							a.cache[q].normalized.push(v)
						}
						H(c, d, f)
					}
					...
					a.stopPropagation();
					f.destroy(b, c, d)
				}).bind("resetToLoadState" + a.namespace, function(c) {
					f.refreshWidgets(b, !0, !0);
					a = g.extend(!0, f.defaults, a.originalSettings);
					b.hasInitialized = !1;
					f.setup(b, a)
				})
			}
			...
			f.setup = function(b, a) {
				...
				a.$table = h.addClass(f.css.table + " " + a.tableClass + c).attr({
					role: "grid"
				});
				a.$headers = g(b).find(a.selectorHeaders);
				a.namespace = a.namespace ? "." + a.namespace.replace(/\W/g, "") : ".tablesorter" + Math.random().toString(16).slice(2);
				a.$tbodies = h.children("tbody:not(." + a.cssInfoBlock + ")").attr({
					"aria-live": "polite",
					"aria-relevant": "all"
				});
				...
				f.applyWidget(b, !0);
				0 < a.sortList.length ? h.trigger("sorton", [a.sortList,
				{}, !a.initWidgets, !0]) : (G(b), a.initWidgets && f.applyWidget(b, !1));
			...
			f.getColumnData = function(b, a, c, h) {
				if ("undefined" !== typeof a && null !== a) {
					b = g(b)[0];
					var e, d = b.config;
					if (a[c]) return h ? a[c] : a[d.$headers.index(d.$headers.filter('[data-column="' + c + '"]:last'))];
					for (e in a) if ("string" === typeof e && (b = h ? d.$headers.eq(c).filter(e) : d.$headers.filter('[data-column="' + c + '"]:last').filter(e), b.length)) return a[e]
				}
			};
			f.computeColumnIndex = function(b) {
				var a = [],
					c = 0,
					h, e, d, f, l, m, r, v, n, q;
				for (h = 0; h < b.length; h++) for (l = b[h].cells, e = 0; e < l.length; e++) {
					d = l[e];
					f = g(d);
					...
					for (d = 0; d < a[m].length + 1; d++) if ("undefined" === typeof a[m][d]) {
						n = d;
						break
					}
					c = Math.max(n, c);
					...
					for (d = m; d < m + r; d++) for ("undefined" === typeof a[d] && (a[d] = []), q = a[d], f = n; f < n + v; f++) q[f] = "x"
				}
				return c + 1
			};
			...
			f.processTbody = function(b, a, c) {
				b = g(b)[0];
				if (c) return b.isProcessing = !0, a.before('<span class="tablesorter-savemyplace"/>'), c = g.fn.detach ? a.detach() : a.remove();
			...
			f.bindEvents = function(b, a, c) {
				...
				a.find(e.selectorSort).add(a.filter(e.selectorSort)).unbind(["mousedown", "mouseup", "sort", "keyup", ""].join(e.namespace + " ")).bind(["mousedown", "mouseup", "sort", "keyup", ""].join(e.namespace + " "), function(c, f) {
					...
					if (!(1 !== (c.which || c.button) && !/sort|keyup/.test(l) || "keyup" === l && 13 !== c.which || "mouseup" === l && !0 !== f && 250 < (new Date).getTime() - d)) {
						if ("mousedown" === l) return d = (new Date).getTime(), "INPUT" === c.target.tagName ? "" : !e.cancelSelection;
						e.delayInit && n(e.cache) && y(b);
			...
			f.destroy = function(b, a, c) {
				...
				if (b.hasInitialized) {
					...
					var d = g(b),
						e = b.config,
						s = d.find("thead:first"),
						k = s.find("tr." + f.css.headerRow).removeClass(f.css.headerRow + " " + e.cssHeaderRow),
						l = d.find("tfoot:first > tr").children("th, td");
					!1 === a && 0 <= g.inArray("uitheme", e.widgets) && (d.trigger("applyWidgetId", ["uitheme"]), d.trigger("applyWidgetId", ["zebra"]));
					s.find("tr").not(k).remove();
					d.removeData("tablesorter").unbind("sortReset update updateAll updateRows updateCell addRows updateComplete sorton appendCache updateCache applyWidgetId applyWidgets refreshWidgets destroy mouseup mouseleave keypress sortBegin sortEnd resetToLoadState ".split(" ").join(e.namespace + " "));
			...
			f.getTextValue = function(b, a, c) {
				if (c) {
					var d = b ? b.length : 0,
						e = c + a;
					for (c = 0; c < d; c++) e += b.charCodeAt(c);
					return a * e
				}
				return 0
			};
			...
			f.replaceAccents = function(b) {
				...
				if (!f.characterRegex) {
					...
					for (a in d)"string" === typeof a && (c += d[a], f.characterRegexArray[a] = new RegExp("[" + d[a] + "]", "g"));
					f.characterRegex = new RegExp(c + "]")
				}
				if (f.characterRegex.test(b)) for (a in d)"string" === typeof a && (b = b.replace(f.characterRegexArray[a], a));
				return b
			};
			...
			f.addParser = function(b) {
				var a, c = f.parsers.length,
					d = !0;
				for (a = 0; a < c; a++) f.parsers[a].id.toLowerCase() === b.id.toLowerCase() && (d = !1);
				d && f.parsers.push(b)
			};
			f.getParserById = function(b) {
				if ("false" == b) return !1;
				var a, c = f.parsers.length;
				for (a = 0; a < c; a++) if (f.parsers[a].id.toLowerCase() === b.toString().toLowerCase()) return f.parsers[a];
				return !1
			};
			...
			f.getWidgetById = function(b) {
				...
				for (a = 0; a < d; a++) if ((c = f.widgets[a]) && c.hasOwnProperty("id") && c.id.toLowerCase() === b.toLowerCase()) return c
			};
			f.applyWidget = function(b, a) {
				...
				!1 !== a && b.hasInitialized && (b.isApplyingWidgets || b.isUpdating) || (c.debug && (n = new Date), c.widgets.length && (b.isApplyingWidgets = !0, c.widgets = g.grep(c.widgets, function(a, b) {
					...
					if (f) {
						if (a || !c.widgetInit[f.id]) f.hasOwnProperty("options") && (d = b.config.widgetOptions = g.extend(!0, {}, f.options, d)), f.hasOwnProperty("init") && f.init(b, f, c, d), c.widgetInit[f.id] = !0;
						!a && f.hasOwnProperty("format") && f.format(b, c, d, !1)
					}
				})), setTimeout(function() {
					b.isApplyingWidgets = !1
				}, 0), c.debug && (k = c.widgets.length, v("Completed " + (!0 === a ? "initializing " : "applying ") + k + " widget" + (1 !== k ? "s" : ""), n)))
			};
			f.refreshWidgets = function(b, a, c) {
				...
				var h, e = b.config,
					...
					k = f.widgets,
					l = k.length;
				for (h = 0; h < l; h++) k[h] && k[h].id && (a || 0 > g.inArray(k[h].id, n)) && (e.debug && d('Refeshing widgets: Removing "' + k[h].id + '"'), k[h].hasOwnProperty("remove") && e.widgetInit[k[h].id] && (k[h].remove(b, e, e.widgetOptions), e.widgetInit[k[h].id] = !1));
			...
			f.getData = function(b, a, c) {
				...
				f = " " + (b.attr("class") || "");
				"undefined" !== typeof b.data(c) || "undefined" !== typeof b.data(c.toLowerCase()) ? d += b.data(c) || b.data(c.toLowerCase()) : e && "undefined" !== typeof e[c] ? d += e[c] : a && "undefined" !== typeof a[c] ? d += a[c] : " " !== f && f.match(" " + c + "-") && (d = f.match(new RegExp("\\s" + c + "-([\\w-]+)"))[1] || "");
				return g.trim(d)
			};
	...
	p.addParser({
		...
		format: function(d, v) {
			var n = p.formatFloat((d || "").replace(/[^\w,. \-()]/g, ""), v);
			return d && "number" === typeof n ? n : d ? g.trim(d && v.config.ignoreCase ? d.toLocaleLowerCase() : d) : d
		},
		type: "numeric"
	});
	p.addParser({
	...
	p.addParser({
		...
		format: function(d, g) {
			var n, w = d ? d.split(".") : "",
				u = "",
				y = w.length;
			for (n = 0; n < y; n++) u += ("00" + w[n]).slice(-3);
			return d ? p.formatFloat(u, g) : d
		},
		type: "numeric"
	});
	p.addParser({
	...
	p.addParser({
		...
		format: function(d, g, n, w) {
			if (d) {
				n = g.config;
				var u = n.$headers.filter("[data-column=" + w + "]:last");
				w = u.length && u[0].dateFormat || p.getData(u, p.getColumnData(g, n.headers, w), "dateFormat") || n.dateFormat;
				d = d.replace(/\s+/g, " ").replace(/[\-.,]/g, "/");
				"mmddyyyy" === w ? d = d.replace(/(\d{1,2})[\/\s](\d{1,2})[\/\s](\d{4})/, "$3/$1/$2") : "ddmmyyyy" === w ? d = d.replace(/(\d{1,2})[\/\s](\d{1,2})[\/\s](\d{4})/, "$3/$2/$1") : "yyyymmdd" === w && (d = d.replace(/(\d{4})[\/\s](\d{1,2})[\/\s](\d{1,2})/, "$1/$2/$3"))
			}
	...
	p.addWidget({
		id: "zebra",
		priority: 90,
		format: function(d, v, n) {
			var w, u, y, z, C, D, E = new RegExp(v.cssChildRow, "i"),
				B = v.$tbodies;
			v.debug && (C = new Date);
			for (d = 0; d < B.length; d++) w = B.eq(d), D = w.children("tr").length, 1 < D && (y = 0, w = w.children("tr:visible").not(v.selectorRemove), w.each(function() {
				u = g(this);
				E.test(this.className) || y++;
				z = 0 === y % 2;
				u.removeClass(n.zebra[z ? 1 : 0]).addClass(n.zebra[z ? 0 : 1])
			}));
			v.debug && p.benchmark("Applying Zebra widget", C)
		},
		remove: function(d, p, n) {
			var w;
			p = p.$tbodies;
			var u = (n.zebra || ["even", "odd"]).join(" ");
			for (n = 0; n < p.length; n++) w = g.tablesorter.processTbody(d, p.eq(n), !0), w.children().removeClass(u), g.tablesorter.processTbody(d, w, !1)
		}
	})
}(jQuery);
