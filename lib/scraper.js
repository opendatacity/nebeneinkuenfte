...
//						"kontakt": [],
					...
					scraper.scrape(_data.url, "html", "utf8", function(err, $){
						...
						if (err) {
							...
							if (/\* ausgeschieden/.test($('.biografie p', '#inhaltsbereich').last().text())) {
								$exit = $('.biografie', '#inhaltsbereich').text().match(/(mit Ablauf des| am) ([0-9]+). ([A-Za-zä]+) ([0-9]{4})( aus dem Deutschen Bundestag ausgeschieden|, Nachfolger| verstorben)/)
								if ($exit) {
									_data.ended = moment({
										year: parseInt($exit[4],10),
										month: $months.indexOf($exit[3]),
										day: parseInt($exit[2],10),
										hour: 23,
										minute: 59,
										second: 59
									}).toISOString();
								} else {
									if (_verbose) console.error("[warn]".inverse.bold.yellow, "no exit date".white, _section.yellow, "for".white, _data.name.yellow, "at".white, _data.url.red);
								}
							}
...
//											console.log($t.html());
										...
										$t.text().split(/\s{5,}/).forEach(function(t){									
											if (t !== "") _capture.lines.push(t);
										});
									}
...
//								if (_data.name === "Dr. Peter Gauweiler") console.log(item);
								_data["nebeneinkuenfte"] = _data["nebeneinkuenfte"].concat(parse_taetigkeiten(item));
							});
							_data["nebeneinkuenfte"].filter(function(i){
								return (i !== null);
							});
							if (_data.ended) _data["nebeneinkuenfte"] = _data["nebeneinkuenfte"].map(function(i){
								if (i.periodical !== "einmalig") i.end = _data.ended;
								return i;
							});
